...
def _force_local_pkg(pkg_name: str, local_root: str) -> None:
    # Drop any already-imported modules so we can re-import from local_root.
    ...
    if os.path.isdir(local_pkg_path):
        # Force namespace package to only use local path.
        mod.__path__ = [local_pkg_path]
        mod_paths = list(mod.__path__)
    if mod_file and local_root not in mod_file:
        raise RuntimeError(f"{pkg_name} imported from non-local path: {mod_file}")
    if mod_paths and any(local_root not in p for p in mod_paths):
...
parser = argparse.ArgumentParser(description="Train modular policy with frozen leg + train arm only.")
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Arm control enable (fixed to 1 by this script).",
)
parser.add_argument(
    "--enable_leg_update",
    type=int,
    default=0,
    help="Leg policy update enable (fixed to 0 by this script).",
)
parser.add_argument(
    "--leg_pt_path",
    type=str,
    default="/home/user/Downloads/LearningHumanoidArmMotion-RAL2025-Code-git/logs/rsl_rl/Tocabi_Arm_Motion/exported/2026-03-09_13-27-45_model_30000_leg.pt",
    help="Path to frozen leg TorchScript policy (.pt) used for leg action inference.",
)
parser.add_argument(
    "--leg_onnx_path",
    type=str,
    default=None,
    help="Path to frozen leg ONNX policy used for leg action inference.",
)
# benchmark options
...
def main():
    ...
    # Force "arm-only training": leg frozen, arm learning enabled.
    if hasattr(agent_cfg, "enable_arm"):
        agent_cfg.enable_arm = True
    if hasattr(agent_cfg, "arm_update_enabled"):
        agent_cfg.arm_update_enabled = True
    if hasattr(env_cfg, "enable_arm"):
        env_cfg.enable_arm = True
    if hasattr(agent_cfg, "leg_update_enabled"):
        agent_cfg.leg_update_enabled = False
    agent_cfg.leg_frozen_pt_path = args_cli.leg_pt_path
    agent_cfg.leg_frozen_onnx_path = args_cli.leg_onnx_path
    if not agent_cfg.leg_frozen_pt_path and not agent_cfg.leg_frozen_onnx_path:
        raise ValueError("Either --leg_pt_path or --leg_onnx_path is required for train_modular_only_arm.py")
    ...
    # Disable leg reward term computation entirely (RewardManager skips terms with zero weight).
    leg_rewards_cfg = getattr(getattr(env_cfg, "rewards", None), "leg", None)
    if leg_rewards_cfg is not None:
        for term_name in dir(leg_rewards_cfg):
            if term_name.startswith("_"):
                continue
            try:
                term_cfg = getattr(leg_rewards_cfg, term_name)
            except Exception:
                continue
